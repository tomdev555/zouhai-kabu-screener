...
評価軸の定義 (重み・0-100スコアの出し方・数値が弱いときの理由) は `src/lib/screening/factors.ts` に集約し、
総合スコアの計算・一覧のポップアップ・閲覧者ごとの「マイスコア」がすべて同じ定義を参照する。
判定ロジックは `src/lib/screening/rules.ts`、実行エンジンは `src/lib/screening/engine.ts`。

**気になる点のポップアップ** — 条件クリアの銘柄でも、内訳が弱い項目があれば一覧の判定欄に⚠を出し、
押すと理由を表示する (例: 「配当利回り3.2%は理想の4%から0.8ポイント低め」「通期が減収 (前年比 -11.75%)」)。

**マイスコア** — 閲覧者が8つの評価軸それぞれの重視度を0〜5で決めると、その重みで計算し直した点数が
「マイスコア」列に出る。並び替えでも使える。設定はブラウザのlocalStorageにのみ保存される。
